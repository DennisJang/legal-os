  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-md bg-black/40"
      ...
      <div onClick={(e) => e.stopPropagation()}
           className="w-[480px] rounded-[24px] bg-[#111]/90 border border-white/20 overflow-hidden
                      shadow-[0_0_64px_rgba(255,255,255,0.08)]
                      animate-in fade-in zoom-in-95 duration-75 ease-out">
        <input ref={inputRef} value={query} onChange={(e) => setQuery(e.target.value)}
          placeholder="검색하거나 명령어를 입력하세요..."
          className="w-full bg-transparent px-[24px] py-[20px] text-[16px] outline-none
                     border-b border-white/10 placeholder:text-white/30
                     focus:ring-1 focus:ring-white/40 transition-all" />
        <ul className="py-[8px]">
          {filtered.map((a) => (
            <li key={a.label}>
        ...
        <div className="px-[24px] py-[12px] border-t border-white/10">
          <span className="text-[11px] text-white/30">ESC로 닫기 · ⌘K 토글</span>
        </div>
      </div>
    </div>
  );
