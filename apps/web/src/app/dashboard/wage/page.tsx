// apps/web/src/app/dashboard/wage/page.tsx
"use client";
...
const StatCard = ({ label, value, sub, color }: { label: string; value: string; sub?: string; color: string }) => (
  <div style={{
    background: T.surface, borderRadius: 14, padding: "14px 16px", flex: 1,
  }}>
...
export default function WagePage() {
  ...
  const isUnderpaid  = totalPay > 0 && totalPay < 2_060_740; // 2025 최저임금 기준
      ...
      <div style={{ maxWidth: 430, margin: "0 auto", paddingInline: 20, paddingTop: 24 }}>
        ...
        {isUnderpaid && (
          <div style={{
            ...
            <div>
              <p style={{ fontSize: 14, fontWeight: 700, color: T.red, letterSpacing: "-0.022em" }}>
                최저임금 미달 가능성
              </p>
        ...
        <div style={{
          ...
          <StatCard
            label="이번 달 예상"
            value={`₩${(totalPay / 10000).toFixed(0)}만`}
            sub="세전 기준"
            color={isUnderpaid ? T.red : T.green}
          />
          <StatCard
            label="출근일"
            value={`${workedDays}일`}
          ...
          <StatCard
            label="초과근무"
            value={`₩${(overtimePay / 10000).toFixed(0)}만`}
            sub="야간 포함"
            color={T.amber}
          />
        </div>
        ...
        <div style={{
          marginBottom: 16,
          animation: "fadeSlideUp 380ms 80ms cubic-bezier(0.32,0.72,0,1) both",
        }}>
          <WageCalendarWidget />
        </div>
        ...
        {/* 진정서 발급 CTA */}
        <div style={{
          animation: "fadeSlideUp 380ms 160ms cubic-bezier(0.32,0.72,0,1) both",
        }}>
          <button
            className="press"
            onClick={() => window.location.href = "/fax"}
            style={{
