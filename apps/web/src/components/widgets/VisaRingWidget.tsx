...
export default function VisaRingWidget() {
  ...
  const pct = Math.min(currentScore / targetScore, 1);
  const r = 54;
  const circ = 2 * Math.PI * r;
  const dash = circ * pct;

  return (
    <div className="relative flex flex-col items-center gap-8 rounded-[24px] bg-[#111] border border-white/10 p-8">
      <div className="relative w-[144px] h-[144px]">
        <svg viewBox="0 0 144 144" className="rotate-[-90deg]" width={144} height={144}>
          ...
          <circle
            cx={72} cy={72} r={r} fill="none" stroke="#fff" strokeWidth={10}
            strokeLinecap="round"
      ...
      <button
        onClick={() => updateVisaScore(5)}
        className="w-full py-[12px] rounded-[16px] bg-white text-[#0A0A0A] text-[14px] font-bold tracking-tight
