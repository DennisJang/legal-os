...
export default function LiabilityActionSheet({ isOpen, onClose, onConfirm }: Props) {
  ...
  const sheetBg    = isDark ? "rgba(28,28,30,0.96)"    : "rgba(255,255,255,0.72)";
  const handleBg   = isDark ? "rgba(255,255,255,0.12)" : "#E5E5EA";
  const titleColor = isDark ? "#FFFFFF"                 : "#1D1D1F";
  const bodyColor  = isDark ? "#A1A1A6"                 : "#86868B";
  const toggleText = isDark ? "#FFFFFF"                 : "#1D1D1F";
  const toggleOff  = isDark ? "#3A3A3C"                 : "#E5E5EA";
  ...
  return createPortal(
    ...
    >
      ...
      >
        {/* Handle */}
        <div style={{ width: 36, height: 4, borderRadius: 2, backgroundColor: handleBg, margin: "0 auto 20px" }} />

        <h2 style={{ fontSize: 24, fontWeight: 700, letterSpacing: "-0.02em", color: titleColor, marginBottom: 8 }}>
          최종 확인 및 법적 책임 동의
        </h2>
        <p style={{ fontSize: 13, color: bodyColor, lineHeight: 1.5, marginBottom: 24, letterSpacing: "-0.01em" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
          <span style={{ fontSize: 17, fontWeight: 400, letterSpacing: "-0.022em", color: toggleText }}>
            위 내용에 동의합니다
          </span>
          <button
            ...
            style={{
              ...
              background: agreed ? "#34C759" : toggleOff,
